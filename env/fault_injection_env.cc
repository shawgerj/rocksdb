...
Status TestWritableFile::Sync() {
  if (!env_->IsFilesystemActive()) {
    return Status::IOError("FaultInjectionEnv: not active");
  }
...
Status FaultInjectionEnv::NewDirectory(const std::string& name,
...
Status FaultInjectionEnv::ReopenWritableFile(
    const std::string& fname, std::unique_ptr<WritableFile>* result,
    const EnvOptions& soptions) {
  if (!IsFilesystemActive()) {
    return GetError();
...
Status FaultInjectionEnv::DeleteFile(const std::string& f) {
  if (!IsFilesystemActive()) {
    return GetError();
  }
...
Status FaultInjectionEnv::RenameFile(const std::string& s,
                                         const std::string& t) {
  if (!IsFilesystemActive()) {
    return GetError();
  }
...
void FaultInjectionEnv::WritableFileClosed(const FileState& state) {
  MutexLock l(&mutex_);
...
void FaultInjectionEnv::WritableFileSynced(const FileState& state) {
  MutexLock l(&mutex_);
  if (open_files_.find(state.filename_) != open_files_.end()) {
...
void FaultInjectionEnv::WritableFileAppended(const FileState& state) {
  MutexLock l(&mutex_);
  if (open_files_.find(state.filename_) != open_files_.end()) {
    if (db_file_state_.find(state.filename_) == db_file_state_.end()) {
      db_file_state_.insert(std::make_pair(state.filename_, state));
...
Status FaultInjectionEnv::DropFileData(
...
Status FaultInjectionEnv::DropRandomUnsyncedFileData(Random* rnd) {
  return DropFileData([&](Env* env, const FileState& state) {
...
void FaultInjectionEnv::UntrackFile(const std::string& f) {
  MutexLock l(&mutex_);
